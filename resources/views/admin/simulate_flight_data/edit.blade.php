...
<div class="container mt-4">
    ...
    <form action="{{ route('admin.simulate_flight_data.update', $flight->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <select name="drone_id" id="drone_id" class="form-select" required>
                ...
                <option value="{{ $drone->id }}"
                        {{ old('drone_id', $flight->drone_id) == $drone->id ? 'selected' : '' }}>
                    {{ $drone->name }}
                </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="latitude" id="latitude" step="0.0000001" class="form-control"
                   value="{{ old('latitude', $flight->latitude) }}" required>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="longitude" id="longitude" step="0.0000001" class="form-control"
                   value="{{ old('longitude', $flight->longitude) }}" required>
        ...
        <div class="mb-3">
            ...
            <input type="number" name="heading" id="heading" step="0.1" class="form-control"
                   value="{{ old('heading', $flight->heading) }}">
            <div class="form-text">Направление движения в градусах от <code>0</code> до <code>360</code>.</div>
        </div>

        <button type="submit" class="btn btn-success">Сохранить</button>
        <a href="{{ route('admin.simulate_flight_data.index') }}" class="btn btn-secondary">Отмена</a>
    </form>
</div>
@endsection
