...
<form method="POST" action="{{ route('admin.simulate_targets.update', $target->id) }}">
    @csrf
    @method('PUT')
    ...
    <div class="mb-3">
        ...
        <input type="text" name="latitude" id="latitude"
               value="{{ old('latitude', $target->latitude) }}"
               class="form-control" placeholder="Напр., 50.4500" required>
    </div>

    <div class="mb-3">
        <label for="longitude" class="form-label">Долгота</label>
        <input type="text" name="longitude" id="longitude"
               value="{{ old('longitude', $target->longitude) }}"
               class="form-control" placeholder="Напр., 30.5233" required>
    </div>
